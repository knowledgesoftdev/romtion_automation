...
export const DataSplit: React.FC<Props> = ({ projectId, channelId, paragraph, durationInFrames }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG, opacity, display: 'flex', flexDirection: 'row' }}>
      <div style={{
        ...
        {sc?.kind === 'stat' && (
          <StatBlock value={sc.payload.value} unit={sc.payload.unit} label={sc.payload.label} accent={accent} channelId={channelId} />
        )}
        {sc?.kind === 'chart' && (
          <BarChart bars={sc.payload.bars} max={sc.payload.max} accent={accent} channelId={channelId} />
        )}
        {sc?.kind === 'bullets' && (
          <Bullets items={sc.payload.items} accent={accent} baseDelay={22} channelId={channelId} />
        )}
        {sc?.kind === 'quote' && (
          <div style={{
            ...
            <div style={{
              ...
              borderLeft: `4px solid ${isCinematic ? '#64748b' : accent}`, paddingLeft: 24,
            }}>"{sc.payload.text}"</div>
            {sc.payload.author && (
